...
*** other keybinding with *ara-map*
    Uses *hydra* since that would be useful text menu display than nested keymap keys.
    #+begin_src emacs-lisp :tangle no
...
** Cleaner key-binding menu *hydra*
   #+begin_src emacs-lisp
     ;; load -hydra
     (use-package hydra)
   #+end_src
...
*** text menu [ara-map t]
    A conglomerate of useful package functions; Note that the packages should exist for this to work
    correctly!
    
    #+begin_src emacs-lisp
      ;; short menu for manipulating/searching/jumping  text
      (defhydra hydra-text-menu (:color red :hint nil)
	"
      [General text]
      
      ^Text^               ^Jump^           ^Search^           ^Goto
      ^^^^^^-----------------------------------------------------------------
      _cl_: Copy line      _gc_: Character _sl_: buffer        _rf_: Read news
      _cr_: Copy region    _gw_: Word      _sp_: project       _rg_: Read gemini site
      _kl_: Delete line    _gl_: Line
      _kr_: Delete region
      _kL_: Kill line
      _kR_: Kill region
      "
	...
	("q" nil))
      ...
      (define-key ara-map "t" 'hydra-text-menu/body)
    #+end_src

    #+RESULTS:
    : hydra-text-menu

*** windows management [ara-map w]
    Use wiki example for handling windows movement
    #+begin_src emacs-lisp
      ;; use hydra to manage windows
      (defhydra hydra-window (:color red
				     :hint nil)
	"
      [Windows management]
      
      ^Movement^              ^Split^                   ^Delete
      ^^^^^^^^^^-----------------------------------------------------
      _h_: move left          _v_: vertical right        _o_: other window
      _j_: move down          _x_: horizontal below     _da_: select window
      _k_: move up            _|_: vertical move right  _dw_: current window
      _l_: move right         ___: horizontal move down _db_: current buffer
      _a_: select window      ^^^                       _df_: frame
      _s_: swap select window
      _f_: new frame"
	...
	("q" nil))
      ...
      (define-key ara-map "w" 'hydra-window/body)
    #+end_src

*** lsp-mode management [ara-map l]
    Defined better organization of menu for lsp-mode use
    #+begin_src emacs-lisp
      ;; add the lsp-mode hydra menu shortcuts
	...
	("S" lsp-shutdown-workspace))
      ...
      (define-key ara-map "l" 'hydra-lsp/body)
...
*** Main package configuration
	...
	(corfu-count 10)
	(corfu-preselect 'prompt)   ;; auto select first match
	...
	(corfu-max-width 80)  ;; max width in characters
...
*** Add icon to capf listing *kind-icon*
    #+begin_src emacs-lisp
      ;; add the icon for the capf listing
...
*** FIXME Move matched completion to minibuffer
    With *embark* we could process more things with completion such as all the available actions
    summarized by *embark* (delete file, lsp action?)

    #+begin_src emacs-lisp
      ;; move completion data to minibuffer for vertico + orderless + embark processing
    ...
    #+end_src
*** COMMENT Add inline document to capf listing *corfu-doc*
    this is superceeded by *corfu-popupinfo*, which is a built-in extension by *corfu*
    #+begin_src emacs-lisp
      ;; add inline doc for the capf listing
      ;; FIXME move to melpa when its there
      (use-package corfu-doc
	:straight (corfu-doc :type git :host github :repo "galeo/corfu-doc")
	:after corfu
	:hook (corfu-mode . corfu-doc-mode)
	:general (:keymap 'corfu-map
			  ;; this is the manual toggle for documentation popup
			  [remap corfu-show-documentation] #'corfu-doc-toggle	; remap default doc command
...
** Capf backends *cape*
   Useful [[https://kristofferbalintona.me/posts/202203130102/][blog]] on what is *cape* and how to use it.
...
* Documents
** gemini protocol *elder*
   Really simple web based internet site on protocol; control are simple and concise shown below
   taken from the start page or pressing =U=. Note this is a special buffer, so meow mode is on the
   motion state.
...
** rss feed reader *elfeed*
   We'll used this to consume the web things since the www is too *commercialized*
   #+begin_src emacs-lisp
     ;; install the news reader and the associated link
     (use-package elfeed
       :config
       (setq elfeed-feeds
...
*** main configuration
**** Configuration with corfu-cape capfs as the completion
   #+begin_src emacs-lisp
...
**** COMMENT Older working configuration but used with *company*
   #+begin_src emacs-lisp :tangle no
...
***** COMMENT using clojure-mode & clojure-lsp
      Unlike *cider* which is using repl like development; this uses more imperative way of doing
      things.
      #+begin_src emacs-lisp
	;; clojure mode + clojure-lsp binary installed
	(use-package clojure-mode
...
**** Rust
     ...
     With nixos we'll have to add =rust-analyzer= to our build script, and remove =rustup= binary.
     #+begin_src emacs-lisp
       ;; add rust + lsp
       (use-package rust-mode
	 :hook ((rust-mode . (lambda ()(setq indent-tabs-mode nil)))
...
**** Lua
     For nixos we'll install via the =luajit= packages, called =luajitPackages.luarock=. Then
     we could run the following at the shell

     Seems to have problem with nix-store, trying with the current =lua-language-server= lsp server
     used in all ide.
     #+begin_src sh :results output :tangle no
       luarocks install --server=http://luarocks.org/dev lua-lsp --local
     #+end_src

     Add the major mode plus the lsp hook
     #+begin_src emacs-lisp
       ;; install lua
     ...
     #+end_src
**** Nix (system builder)
     Using with lsp doesn't seem to coresspond to what nix interpreter sees; so better to disable it.
     #+begin_src emacs-lisp :tangle no
       ;; install the major mode
       (use-package nix-mode
	 :mode "\\.nix\\'"
     ...
     We'll use the default nix-mode only for the syntax highlight
     #+begin_src emacs-lisp
       ;; nix-mode only
       (use-package nix-mode
	 :mode "\\.nix\\'")
