...
** Code completion *company*
   ...
   3rd party capf provider backends
   #+begin_src emacs-lisp :tangle no
     (use-package company)
   #+end_src

** COMMENT Code completion (require capf backends) *corfu*
...
** COMMENT Capf backends *cape*
   Add additional capf as well as third party capf backends.
   ...
   We could use company backend with cape-company-capf adapter
   #+begin_src emacs-lisp
...
**** Java
	 ...
	 (add-to-list 'eglot-server-programs
		      '((java-mode java-ts-mode) . "jdt-language-server")))
...
** lsp-client *lsp-mode*
   ...
   #+begin_src emacs-lisp :tangle no
	 ...
	 (setf (alist-get 'styles (alist-get 'lsp-capf completion-category-defaults))
	       '(flex))) ;; Configure orderless/flex(corfu)
